...
| Version | Notes |
|---|---|
| 2.5 | New Step Index tab — every step used in the file with usage counts, Commit Records split by dialog on/off, drill-down to the scripts using each step, and a content search to find a dialog by its message. Script rendering rebuilt against the real Save as XML grammar: Set Variable, Set Field, Commit, New Window, GTRR and Go to Record now show their real content. PSoS check rebuilt from the verified platform matrix. New checks for broken value list sources, dead conditional formatting, and blank-name Set Variables. Tables to Fields now scopes field search to the source table. |
